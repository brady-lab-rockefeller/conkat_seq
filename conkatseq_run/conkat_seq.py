...
if __name__ == "__main__":
    ...
    ensure_dir(OUTPATH)
    print('*************')
    print(list_of_clustering_dataframes)
    frames = []
    ...
    if (os.path.isfile(network_compressed) & (os.path.isfile(contraction_df_fullpath))
     & ~(bool(override)) & (bool(merge_similar_id))):
        ...
        contraction_df_fullpath = OUTPATH + 'MERGED_DOMAINS#' + '#'.join(DOMAINS) + '.csv'
        if len(contraction_df) > 0:
            contraction_df.to_csv(contraction_df_fullpath)
    else:
        log('Skipping merge of similar domains within networks...')
